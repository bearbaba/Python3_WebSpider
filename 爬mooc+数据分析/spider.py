class Spider():
    ...
    def save(self):
        ...
        for i in range(1,len(self.el_name_sum)+1):
            sheet.write(i,0,self.el_name_sum[i-1])
            sheet.write(i,1,self.el_lang_sum[i-1])
            sheet.write(i,2,self.el_level_sum[i-1])
            sheet.write(i,3,self.el_heat_sum[i-1])
        workbook.save(r'D:\computer_language\Python_Study\Python_爬虫\爬mooc+数据分析\mooc.xlsx')
...
if __name__ == "__main__":
    ...
    s=Spider()
    pool.map(s.get_responses,s.get_urls())
    pool.close()
    pool.join()
    s.save()
    #s.show()
